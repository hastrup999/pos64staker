...
CHANGE = input("Please specify how much change you want to keep unlocked: ")
if int(CHANGE) < 1:
    sys.exit('Cant have negative change. Try again.')
UTXOS = input("Please specify the amount of UTXOs to send to each segid: ")
AMOUNT = int((float(balance)-float(CHANGE))/(int(UTXOS)*64))
print('Amount per UTXO'+str(AMOUNT))
